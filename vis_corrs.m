function [stats] = vis_corrs(choice)
%
...
minex = input(' Only include values above V? [1] yes; [2] no <ENTER>: ');
if isempty(minex)
   minex = 2;
end
...
if numf > 1
    plotfigs = input('\n Plot scatterplots? [1] yes; [2] no <ENTER>: ');
else
    plotfigs = 1;
end
...
if plotfigs == 1
    if choice == 3
        ...
        lx = xlabel(strcat(namef1b_file)); ly = ylabel(strcat(namef2b_file)); title('Scatter plot'); whitebg('w');
    ...
    elseif choice == 4
        ...
        hold on
        lx = xlabel([namef1b_file ' quantiles']); ly = ylabel([namef2b_file ' quantiles']); title('Quantile-Quantile Plot'); whitebg('w');
        set(lx,'Interpreter','none'); set(ly,'Interpreter','none');
        hold off
    ...
    elseif choice == 5
        ...
        figure(33)   
        plot(ba1,ba2,'+')
        xlabel('(Image2 + Image2)/2');
        ylabel('Image1 - Image2');
...
if numf == 1
    if choice == 3
        fprintf([' The ' num2str(length(v1)) ' image values have a Pearson correlation = ' num2str(rval) '. \n']);
    elseif choice == 4
        fprintf([' The 101 values in the Q-Q plot are correlated with r = ' num2str(quant_r) '. \n']);
    elseif choice == 5
