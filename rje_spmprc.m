function [y] = rje_spmprc(data, p)
%
% * modified by rje to treat each column as independent (aug 2012)
%
...
cols = size(data,2); % rje edit
y = nan(numel(p),cols);
for c = 1:cols
    data2 = data(:,c);

    data2        = sort(data2(~isnan(data2)));
    N           = length(data2);
    data2(end+1) = data(end); % to handle special case of 100th percentile below

    x = 1 + p * (N - 1) / 100;
    k = floor(x);
    d = x - k;

    y(:,c) = (1 - d) .* data2(k) + d .* data2(k + 1);
end
